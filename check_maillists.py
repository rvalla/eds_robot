...
for l in range(len(maillist)):
  ...
  if not check_domain(data[0]):
    errors.append((l + 1, "mail"))
  if not check_list(data[1].split(","), CALENDARS):
    if not data[1].split(",")[0] == data[0]:
      errors.append((l + 1, "suscriptions"))
  if not check_list(data[2].split(","), CALENDARS):
    errors.append((l + 1, "permissions"))
  if not check_number(data[3]):
    errors.append((l + 1, "later_events"))
  if not check_list(data[4].split(","), TAGS):
    errors.append((l + 1, "later_tags"))
  if not check_number(data[5]):
    errors.append((l + 1, "mediadores"))
...
#Loading the daily maillist file...
t_mails_path = "data/csv/" + config["file_prefix"] + "maillist_daily.csv"
maillist = open(t_mails_path).readlines()[1:]
errors = []

print("I will check the daily mailing list now...", end="\n")
for l in range(len(maillist)):
  data = maillist[l].split(";")
  if not check_domain(data[0]):
    errors.append((l + 1, "mail"))
