class HtmlFormat():
  ...
  def event_row(self, event, in_next_week):
    row = "<tr>\n"
    row += self.date_cell(event[3], event[5], in_next_week)
    row += self.summary_cell(event[1], event[2], event[6])
    row += "</tr>\n"
    return row
  
  def date_cell(self, date, interval, in_next_week):
    cell = "<td align=right>" + date.strftime(self.date_format)
    if not date.hour == 0:
      cell += "<br>" + date.strftime(self.time_format)
    if not in_next_week:
      cell += "<br>" + "en " + str(interval) + " días"
    cell += "</td>\n"
    return cell

  def summary_cell(self, summary, description, meet_call):
    cell = "<td colspan=3>"
